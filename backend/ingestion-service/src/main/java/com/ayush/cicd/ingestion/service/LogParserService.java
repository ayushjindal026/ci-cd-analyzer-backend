// PATH: backend/ingestion-service/src/main/java/com/ayush/cicd/ingestion/service/LogParserService.java
...
public class LogParserService {
    ...
    private static final Pattern EXIT_CODE = Pattern.compile(
            "exit(?:ed with| code)[:\\s]+(\\d+)", Pattern.CASE_INSENSITIVE);
    ...
    public ParsedLog parse(String rawLog, String stage) {
        if (rawLog == null || rawLog.isBlank())
            return ParsedLog.empty(stage);

        String clean = TIMESTAMP.matcher(rawLog).replaceAll("");
    ...
    private List<String> extractErrorLines(String[] lines) {
    ...
    private List<String> extractStackTrace(String log) {
    ...
    private List<String> extractFailingTests(String log) {
        List<String> out = new ArrayList<>();
        ...
        return out.stream().distinct().limit(20).collect(Collectors.toList());
    }

    private FailureCategory classify(String log, List<String> tests) {
    ...
    private String extractExitCode(String log) {
        Matcher m = EXIT_CODE.matcher(log);
        return m.find() ? m.group(1) : null;
    }

    private String pickRootCause(List<String> errors, List<String> stack) {
        return stack.stream()
                .filter(l -> l.startsWith("Caused by"))
                .findFirst()
                .orElse(errors.isEmpty() ? null : errors.get(0));
    }

    private String extractFailingStep(String log) {
        Matcher m = STEP_HEADER.matcher(log);
        String last = null;
        while (m.find()) {
            String s = m.group(1) != null ? m.group(1)
    ...
    private String condense(List<String> errors, List<String> stack, int max) {
        List<String> all = new ArrayList<>(errors);
        all.addAll(stack);
        return all.stream().limit(max).collect(Collectors.joining("\n"));
    }

    // ── ParsedLog value object ────────────────────────────────────────────────
    ...
    public static class ParsedLog {
        ...
        public static ParsedLog empty(String stage) {
            return ParsedLog.builder()
                    .stage(stage)
                    .category(FailureCategory.UNKNOWN)
                    .errorLines(List.of()).stackTrace(List.of()).failingTests(List.of())
                    .build();
        }

        public boolean hasFailure() {
            return category != FailureCategory.UNKNOWN
                    || !errorLines.isEmpty() || !stackTrace.isEmpty();
        }
    }
}